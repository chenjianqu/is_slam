...
class FastBaseTransform(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.mean = torch.Tensor(MEANS).float().cuda()[None, :, None, None] #数据集像素均值
        self.std  = torch.Tensor( STD ).float().cuda()[None, :, None, None] #数据集像素标准差
    ...
    def forward(self, img):
        ...
        if cfg.preserve_aspect_ratio:#是否保存图像的长宽比
            _, h, w, _ = img.size()
            img_size = Resize.calc_size_preserve_ar(w, h, cfg.max_size)
            img_size = (img_size[1], img_size[0]) # Pytorch needs h, w
        else:
            img_size = (cfg.max_size, cfg.max_size) #设置缩放后的图像大小550*550

        img = img.permute(0, 3, 1, 2).contiguous() #将图像通道数放到第二维
        img = F.interpolate(img, img_size, mode='bilinear', align_corners=False) #缩放图像
